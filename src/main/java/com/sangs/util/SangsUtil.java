...
public class SangsUtil extends EgovStringUtil{
    ...
    * @return 변환된 문자열
    ...
    // 유효성 검사
    public static String isRegPassword(String userId, String password) {
    	...
    	// 아이디와, 비밀번호 동일한 값인지 아닌지 비교
    	else if(password.equals(userId)) {
    		result = "fail";
    		return result;
    		
    	// 비밀번호 숫자 + 영문자 + 특수문자 1자 이상 기입안했을 경우
    	} else if (!password.isEmpty()) {
    		
    		
    		Matcher matcher = Pattern.compile("^(?=.*[A-Za-z])(?=.*[0-9])(?=.*[-_=+|()*&^%$#@!~`?></;,.:'])[A-Za-z[0-9]-_=+|()*&^%$#@!~`?></;,.:']{8,14}$").matcher(password);
    		boolean matcherChk = matcher.find();
    	...
    	for(int i = 0; i < password.length()-2;) {
    		...
    		char continueStr = password.charAt(i);
    		
			if((continueStr == password.charAt(i)) == true && (continueStr == password.charAt(i+1)) == true && (continueStr == password.charAt(i+2)) == true)  {
				result = "fail";
				return result;
			}
