...
function fnView(mode, bbsNo){
    if(bbsNo == 0){
        ...
        $('#bbsNo').val(bbsNo);
        $('#bbsForm').attr('action', '/academy/helpdesk/boardDataView.do').submit();
    }
}
...
function nextClick(bbsNo){
	if(bbsNo.length != 0){
		$("#bbsNo").val(bbsNo);
		$("#frm").attr("action", "/academy/helpdesk/boardDataView.do").submit();
	}else{
		alert("처음페이지 입니다.");
		return;
	}
...
//목록 버튼 클릭
function mainList(){
	$("#frm").attr("action", "/academy/helpdesk/boardDataList.do").submit();
}

function fnList(){
    $('form[name="bbsForm"]').attr('action', '<c:out value="${ctx}"/>/academy/helpdesk/boardDataList.do').attr('target', '_self').submit();
}
