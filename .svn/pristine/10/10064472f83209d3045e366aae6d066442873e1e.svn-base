...
function fnView(mode, bbsNo){
    if(bbsNo == 0){
        ...
        $('#bbsNo').val(bbsNo);
        $('#bbsForm').attr('action', '/academy/dataRoom/dataroomBoardView.do').submit();
    }
}
...
function nextClick(bbsNo){
	if(bbsNo.length != 0){
		$("#bbsNo").val(bbsNo);
		$("#frm").attr("action", "/academy/dataRoom/dataroomBoardView.do").submit();
	}else{
		alert("처음페이지 입니다.");
		return;
	}
...
//목록 버튼 클릭
function mainList(){
	$("#frm").attr("action", "/academy/dataRoom/dataroomBoardList.do").submit();
}

function fnList(){
    $('form[name="bbsForm"]').attr('action', '<c:out value="${ctx}"/>/academy/dataRoom/dataroomBoardList.do').attr('target', '_self').submit();
}
			...
			<div class="subConts" id="sub-content">
				...
				<div class="contents" id="sContents">
					<form name="frm" id="frm" action="" method="post">
						...
						<input type="hidden" name="searchType" value="<c:out value='${REQUEST_DATA.searchType}'/>"/>
					</form>
											...
											<c:choose>
												...
												<c:when test="${not empty listData}">
													<c:forEach items="${listData}" var="result" varStatus="status">
														<p class="a_file"><a class="a_file" href="#none" onclick="filedown('<c:out value="${result.FILE_ID}" />' , '<c:out value="${result.BBSNO}" />', '<c:out value="${result.FILE_SEQ}" />');return false"class="btn-down" title="첨부파일 다운로드"><c:out value="${result.FILE_NAME}" /></a></p>
													</c:forEach>
												</c:when>
											</c:choose>
										</td>
