...
function move_page(cPage){
	...
	$('form[name=frm]').attr('action', '<c:out value="${ctx}"/>/academy/dataRoom/dataroomBoardList.do').submit();
} 
									...
									<c:when test="${not empty listData}">
										<c:forEach items="${listData}" var="result" varStatus="status">
											<tr>
												...
												<c:if test="${result.BCATENO eq 6 || result.BCATENO eq 1041 || result.BCATENO eq 1042 || result.BCATENO eq 1043}">
													<strong class="fnt16 po_C7">${resultCount-(REQUEST_DATA.cPage-1)*10-status.count+1}</strong>
												</c:if>
												</td>
												<td>
